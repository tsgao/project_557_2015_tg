...
GLSphereGreen::GLSphereGreen(float center_x, float center_y, float center_z, float radius, int rows, int segments )
{
...
void GLSphereGreen::initShader(void)
{
    ...
    _program = LoadAndCreateShaderProgram("../data/shaders/ass3p2green.vs", "../data/shaders/ass3p2green.fs");
#else
	 // This loads the shader program from a file
    _program = LoadAndCreateShaderProgram("../../data/shaders/ass3p2green.vs", "../../data/shaders/ass3p2green.fs");
#endif
	...
	// Spot light 0
	_light_source0._lightPos = glm::vec4(-30.0, 0.0, 15.0, 0.0);
	_light_source0._ambient_intensity = 0.0;
	_light_source0._specular_intensity = 0.0;
	_light_source0._diffuse_intensity = 4.0;
	
	_light_source0._cone_angle = 12; // in degree
	_light_source0._cone_direction = glm::vec3(30, 4, -20); // this must be aligned with the object and light position.
	...
	_light_source0._cone_angleIdx = glGetUniformLocation(_program, "cone_angle0");
	_light_source0._cone_directionIdx = glGetUniformLocation(_program, "cone_direction0");
	...
	// Spot light 1
	_light_source1._lightPos = glm::vec4(-24.0, -1.0, 1.0, 0.0);
	...
	_light_source1._cone_direction = glm::vec3(24.0, 1.0, -1.0); // this must be aligned with the object and light position.
